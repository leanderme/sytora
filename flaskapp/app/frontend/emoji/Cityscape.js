import React from 'react';

export default function Cityscape(props) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" enableBackground="new 0 0 64 64" width={64} height={64} {...props}>
      <path d="m49 30c-3.2 0-9.8 0-13 0-1.8 0-3 .9-3 2.7v31.3h20v-30c0-2.2-1.8-4-4-4" fill="#fbbf67" />
      <g fill="#d3976e">
        <path d="m36.2 35h-.4c-.4 0-.8.4-.8 1v28h2v-28c0-.6-.4-1-.8-1" />
        <path d="m41.2 35h-.4c-.4 0-.8.4-.8 1v28h2v-28c0-.6-.4-1-.8-1" />
      </g>
      <path d="M13,34H4c-2.2,0-4,1.8-4,4v26h13V34z" fill="#6adbc6" />
      <g fill="#fff">
        <path d="m9.5 42c0 .5-.4 1-1 1h-3c-.6 0-1-.5-1-1v-2c0-.6.4-1 1-1h3c.6 0 1 .4 1 1v2" />
        <path d="m9.5 50c0 .5-.4 1-1 1h-3c-.6 0-1-.5-1-1v-2c0-.6.4-1 1-1h3c.6 0 1 .4 1 1v2" />
        <path d="m9.5 58c0 .5-.4 1-1 1h-3c-.6 0-1-.5-1-1v-2c0-.6.4-1 1-1h3c.6 0 1 .4 1 1v2" />
      </g>
      <g fill="#d6eef0">
        <path d="m15 11c-2 0-3.8 1-4.9 2.5-.6-.3-1.3-.5-2.1-.5-2.4 0-4.4 1.7-4.9 4h-.1c-1.7 0-3 1.3-3 3s1.3 3 3 3h12c3.3 0 6-2.7 6-6 0-3.3-2.7-6-6-6" />
        <path d="m36 10c.4 0 .7.1 1.1.2.4-2.4 2.4-4.2 4.9-4.2 2 0 3.7 1.2 4.5 2.9.7-.6 1.6-.9 2.5-.9 2.2 0 4 1.8 4 4s-1.8 4-4 4h-13c-1.7 0-3-1.3-3-3s1.3-3 3-3" />
      </g>
      <path d="m31.6 17.9l-7.2-7.3c-.8-.8-2.1-.8-2.8 0l-7.2 7.3c-.8.8-1.4 2.3-1.4 3.5v42.6h20v-42.7c0-1.1-.6-2.7-1.4-3.4" fill="#d3976e" />
      <g fill="#ffdd7d">
        <path d="m30 34c0 .5-.5 1-1 1h-12c-.5 0-1-.5-1-1v-2c0-.6.5-1 1-1h12c.5 0 1 .4 1 1v2" />
        <path d="m30 42c0 .5-.5 1-1 1h-12c-.5 0-1-.5-1-1v-2c0-.6.5-1 1-1h12c.5 0 1 .4 1 1v2" />
        <path d="m30 50c0 .5-.5 1-1 1h-12c-.5 0-1-.5-1-1v-2c0-.6.5-1 1-1h12c.5 0 1 .4 1 1v2" />
        <path d="m30 58c0 .5-.5 1-1 1h-12c-.5 0-1-.5-1-1v-2c0-.6.5-1 1-1h12c.5 0 1 .4 1 1v2" />
        <path d="m30 26c0 .5-.5 1-1 1h-12c-.5 0-1-.5-1-1v-2c0-.6.5-1 1-1h12c.5 0 1 .4 1 1v2" />
      </g>
      <path d="m60 19v-5c0-1.1-.9-2-2-2h-3v-11c0-.6-.4-1-1-1s-1 .4-1 1v11h-3c-1.1 0-2 .9-2 2v5c-2.2 0-4 1.8-4 4v41h20v-41c0-2.2-1.8-4-4-4" fill="#6adbc6" />
      <g fill="#fff">
        <path d="m52.5 25c0 .5-.5 1-1 1h-3c-.5 0-1-.5-1-1v-2c0-.6.5-1 1-1h3c.5 0 1 .4 1 1v2" />
        <path d="m60.5 25c0 .5-.5 1-1 1h-3c-.5 0-1-.5-1-1v-2c0-.6.5-1 1-1h3c.5 0 1 .4 1 1v2" />
        <path d="m52.5 32c0 .5-.5 1-1 1h-3c-.5 0-1-.5-1-1v-2c0-.6.5-1 1-1h3c.5 0 1 .4 1 1v2" />
        <path d="m60.5 32c0 .5-.5 1-1 1h-3c-.5 0-1-.5-1-1v-2c0-.6.5-1 1-1h3c.5 0 1 .4 1 1v2" />
        <path d="m52.5 39c0 .5-.5 1-1 1h-3c-.5 0-1-.5-1-1v-2c0-.6.5-1 1-1h3c.5 0 1 .4 1 1v2" />
        <path d="m60.5 39c0 .5-.5 1-1 1h-3c-.5 0-1-.5-1-1v-2c0-.6.5-1 1-1h3c.5 0 1 .4 1 1v2" />
        <path d="m52.5 46c0 .5-.5 1-1 1h-3c-.5 0-1-.5-1-1v-2c0-.6.5-1 1-1h3c.5 0 1 .4 1 1v2" />
        <path d="m60.5 46c0 .5-.5 1-1 1h-3c-.5 0-1-.5-1-1v-2c0-.6.5-1 1-1h3c.5 0 1 .4 1 1v2" />
        <path d="m52.5 53c0 .5-.5 1-1 1h-3c-.5 0-1-.5-1-1v-2c0-.6.5-1 1-1h3c.5 0 1 .4 1 1v2" />
        <path d="m60.5 53c0 .5-.5 1-1 1h-3c-.5 0-1-.5-1-1v-2c0-.6.5-1 1-1h3c.5 0 1 .4 1 1v2" />
        <path d="m52.5 60c0 .5-.5 1-1 1h-3c-.5 0-1-.5-1-1v-2c0-.6.5-1 1-1h3c.5 0 1 .4 1 1v2" />
        <path d="m60.5 60c0 .5-.5 1-1 1h-3c-.5 0-1-.5-1-1v-2c0-.6.5-1 1-1h3c.5 0 1 .4 1 1v2" />
      </g>
    </svg>
  );
}
